...
# Add compact-common to path
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '../../compact-common/src')))
try:
    from compact_common.eos import SigmaOmegaModel
    from compact_common.structure import TOVSolver
    USING_COMPACT_COMMON = True
except ImportError:
    print("Warning: compact_common not found. Using local implementation.")
    USING_COMPACT_COMMON = False
...
def compute_epsilon_p1(Gs, Gv):
    if USING_COMPACT_COMMON:
        model = SigmaOmegaModel(Gs, Gv)
        # Using the same range as kf_list implies (0.0001, 5)
        energy_density, pressure = model.compute_eos(number_density_range=(0.0001, 5.0), num_points=300)
        return list(energy_density), list(pressure)
    else:
        # Local implementation
        return _compute_epsilon_p1_local(Gs, Gv)

@jit(nopython=True)    
def _compute_epsilon_p1_local(Gs, Gv):
    sigma_list = []
    M_star_list = []
    epsilon_list = []
    p_list = []
...
def TOV_M_R(cen_dens, pressure1, density1):
    if USING_COMPACT_COMMON:
        # pressure1 and density1 from compute_epsilon_p1 are in MeV/fm^3
        # TOVSolver handles unit conversion internally if we are consistent
        # But wait, TOVSolver assumes input is in MeV/fm^3 for energy density and pressure?
        # My implementation of TOVSolver in previous turn: 
        # "Unit conversion to cgs for calculation if input is MeV/fm^3"
        # self.p_cgs = self.p_arr * MEV_FM3_TO_DYNE_CM2
        # So yes, it expects MeV/fm^3.
        
        # cen_dens passed here is in geometric units (m^-2) because it was converted in the calling code?
        # Let's check the calling code in likelihood_transform_:
        # ep=np.array(ep)*Mev_fm3_to_GU
        # pr=np.array(pr)*Mev_fm3_to_GU
        # cen_dens=np.logspace(...) based on ep.
        # So cen_dens is in GU.
        
        # This is a mismatch. TOVSolver expects MeV/fm^3 or CGS?
        # If I use TOVSolver, I should pass density/pressure in CGS or MeV/fm^3.
        # The script converts to GU before calling TOV_M_R.
        
        # To make this work without rewriting the whole script's unit logic:
        # I can inverse convert GU to MeV/fm^3, or instantiate TOVSolver with GU units if I adjust constants.
        # Easier: Convert inputs back to MeV/fm^3 or CGS before passing to TOVSolver.
        
        # Mev_fm3_to_GU = 1.6* 10**32 * G1/c1**4
        # Inverse: GU_to_Mev_fm3 = 1/Mev_fm3_to_GU
        
        # Wait, if I change TOV_M_R, I need to handle the units.
        # pressure1 and density1 passed to this function are already in GU (from likelihood_transform_).
        
        # Let's assume we convert GU back to MeV/fm^3 for TOVSolver.
        GU_to_Mev_fm3 = 1.0 / Mev_fm3_to_GU
        
        p_mev = np.array(pressure1) * GU_to_Mev_fm3
        e_mev = np.array(density1) * GU_to_Mev_fm3
        
        solver = TOVSolver(e_mev, p_mev)
        
        M1 = []
        R1 = []
        
        # cen_dens is in GU. Convert to g/cm^3 for TOVSolver.solve
        # 1 GU (1/m^2) corresponds to density?
        # In original code: M/M01, R/1000.
        # M01 = 1.474e3 m.
        # TOVSolver returns R in km, M in M_sun.
        # R in km is R/1000 if R is in meters.
        # M in M_sun.
        
        # We need to convert cen_dens (GU) to g/cm^3.
        # 1 MeV/fm^3 = 1.78e12 g/cm^3.
        # 1 MeV/fm^3 = Mev_fm3_to_GU GU.
        # So 1 GU = (1.78e12 / Mev_fm3_to_GU) g/cm^3.
        
        factor = Mev_fm3_to_gcc / Mev_fm3_to_GU
        
        for d_gu in cen_dens:
            rho_c_gcc = d_gu * factor
            result = solver.solve(rho_c_gcc)
            if result:
                r_km, m_sun = result
                M1.append(m_sun) # Original returns m/M01 (M_sun)
                R1.append(r_km)  # Original returns r/1000 (km)
            else:
                M1.append(0)
                R1.append(0)
        ...
        return _TOV_M_R_local(cen_dens, pressure1, density1)

@jit(nopython = True)
def _TOV_M_R_local(cen_dens, pressure1, density1):
